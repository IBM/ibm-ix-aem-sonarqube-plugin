...
class CloseJcrSessionTest {

    @Test
    @DisplayName("GIVEN a method, WHEN using a JCR session and it is not closed, THEN the test should verify that this is not compliant")
    void detected() {
        CheckVerifier
          .newVerifier()
          .onFile(TestConstants.SLING_TEST_PATH + "CloseJcrSessionCheck.java")
          .withCheck(new CloseJcrSessionRule())
