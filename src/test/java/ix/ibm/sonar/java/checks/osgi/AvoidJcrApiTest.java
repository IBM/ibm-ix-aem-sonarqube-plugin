...
class AvoidJcrApiTest {

    @Test
    @DisplayName("GIVEN a method, WHEN the method uses the JCR API JcrUtil, THEN the test should verify that this is not compliant")
    void detected() {
        CheckVerifier
          .newVerifier()
          .onFiles(TestConstants.OSGI_TEST_PATH + "AvoidJcrApiCheck.java")
          .withCheck(new AvoidJcrApiRule())
