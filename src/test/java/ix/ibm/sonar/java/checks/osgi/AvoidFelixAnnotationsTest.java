...
class AvoidFelixAnnotationsTest {

    @Test
    @DisplayName("GIVEN a class, WHEN Apache Felix components annotations are used, THEN the test should verify that both are not compliant")
    void detected() {
        CheckVerifier
          .newVerifier()
          .onFiles(TestConstants.OSGI_TEST_PATH + "AvoidFelixAnnotationsCheck.java")
          .withCheck(new AvoidFelixAnnotationsRule())
