...
import ix.ibm.sonar.java.utils.*;
...
public class FilterExceptionRule extends ExpandedTreeVisitor {
    ...
    private static final String[] UNEXPECTED_BEHAVIOUR_EXCEPTIONS = new String[] { PackageConstants.IO_EXCEPTION, PackageConstants.JAVAX_SERVLET_EXCEPTION };
    ...
    private void checkCaughtExceptions(final TryStatementTree tryStatementTree) {
        for (final CatchTree catchTree : tryStatementTree.catches()) {
            if (JavaFinder.anyOfTheExceptionsBeingCaught(catchTree, UNEXPECTED_BEHAVIOUR_EXCEPTIONS)) {
